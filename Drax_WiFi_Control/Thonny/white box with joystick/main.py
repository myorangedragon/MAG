# WiFi Motor Control Client 'GS_Joy_Con_Tx_16.py'
...
# Flashed to ESP32 D32 3/12/21 ~15:32
...
pc = Pin(19, Pin.IN, Pin.PULL_UP)# GPIO 19 up  (c,C)
pd = Pin(22, Pin.IN, Pin.PULL_UP)# GPIO 22 down (d,D)
...
wlan.active(True)
wlan.config(dhcp_hostname='GS_Joy_Con_Tx_16')
...
#     if pns.value() == 0: Now redundant, was for reversing up/down.
...
#         pd = Pin(22, Pin.IN, Pin.PULL_UP)
    ...
    if count > 0 and count < 6:
        pled.value (1) # flashes FP LED to show running
        pulse.value (0) # flashes internal LED to show running
    if count > 5:
        pled.value (0)
        pulse.value (1)
    if count > 60:
        print (data) # For testing with Termite
        count = 0
